...
public class  PropertiesLoader {
    ...
    public PropertiesLoader(String... propertiesPath) {/*可以将多个properties文件载入*/
    ...
    private Properties loadProperties(String... resourcePath){
        ...
        for (String path:resourcePath) {
            ...
            try {
                ...
                properties.load(is);//读取到属性文件中
            } catch (IOException e) {
                logger.info("属性文件加载出现异常 path:"+path+"  异常信息:"+e.getMessage());
            }finally {
                IOUtils.closeQuietly(is);//关闭资源 不需要再次try catch 精简代码
            }
        }
        return  properties;
    }
